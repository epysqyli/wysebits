...
const getFeed = async (page) => {
  return await axios({
    method: "get",
    url: `http://localhost:3001/api/tile_entries/feed?page=${page}`,
  });
};

const getCustomFeed = async (user, context, page) => {
  return await axios({
    method: "get",
    url: `http://localhost:3001/api/users/${user.data.user.id}/custom_feed?page=${page}`,
    headers: { cookie: context.req.headers.cookie },
  });
};

const getCategoryRecommendations = async (category) => {
  return await axios.get(
    `http://localhost:3001/api/categories/${category.id}/recommendations`
  );
};
