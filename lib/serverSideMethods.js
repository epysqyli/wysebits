...
const getCategoryBooks = async (slug, pageNum = 1) => {
  return await axios.get(
    `${process.env.BASE_URL}/categories/${slug}?page=${pageNum}`
  );
};
...
const getBookEntries = async (id, pageNum) => {
  return axios(
    `${process.env.BASE_URL}/books/${id}/tile_entries?page=${pageNum}`
  );
};

const getUserEntries = async (user, pageNum) => {
  return await axios.get(
    `${process.env.BASE_URL}/users/${user.data.user.id}/tile_entries?page=${pageNum}`
  );
};
...
const getCategoryRecommendations = async (bookId) => {
  return await axios.get(
    `${process.env.BASE_URL}/books/${bookId}/recommendations`
  );
};
