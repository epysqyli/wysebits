...
const addToFavInsight = async (user, entry) => {
  await axios({
    method: "POST",
    url: `http://localhost:3001/api/users/${user.id}/fav_tile_entries`,
    data: { id: entry.id },
...
const removeFromFavInsights = async (user, entry) => {
  await axios({
    method: "DELETE",
    url: `http://localhost:3001/api/users/${user.id}/fav_tile_entries/${entry.id}`,
    withCredentials: true,
  });
};
...
const upvote = async (user, entry) => {
  await axios({
    method: "POST",
    url: `http://localhost:3001/api/users/${user.id}/upvoted_entries`,
    data: { id: entry.id },
    withCredentials: true,
  });
};
...
const removeUpvote = async (user, entry) => {
  await axios({
    method: "DELETE",
    url: `http://localhost:3001/api/users/${user.id}/upvoted_entries/${entry.id}`,
    withCredentials: true,
  });
};
...
const downvote = async (user, entry) => {
  await axios({
    method: "POST",
    url: `http://localhost:3001/api/users/${user.id}/downvoted_entries`,
    data: { id: entry.id },
    withCredentials: true,
  });
};
...
const removeDownvote = async (user, entry) => {
  await axios({
    method: "DELETE",
    url: `http://localhost:3001/api/users/${user.id}/downvoted_entries/${entry.id}`,
    withCredentials: true,
  });
};
