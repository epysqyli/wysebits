...
const addInsightToState = (insight, insights, setState) => setState([...insights, { id: insight.id }]);
...
const addToFavInsightsAndUpdateState = async (user, entry, insights, setState) => {
...
const removeFromFavInsightsAndUpdateState = async (user, entry, insights, setState) => {
...
const removeDownEntryFromState = (entry, downvotedEntries, setState) => {
  entry.downvotes -= 1;
  const newDownvotedEntries = downvotedEntries.filter((el) => el.id !== entry.id);
...
const downvoteAndUpdateState = async (
  ...
  if (isUpvoted(upvotedEntries, entry)) removeUpvoteAndUpdateState(user, entry, upvotedEntries, setUpvoted);
  addDownEntryToState(entry, downvotedEntries, setDownvoted);
};

const removeUpvoteAndUpdateState = async (user, entry, upvotedEntries, setState) => {
  await removeUpvote(user, entry);
  removeUpEntryFromState(entry, upvotedEntries, setState);
};

const removeDownvoteAndUpdateState = async (user, entry, downvotedEntries, setState) => {
