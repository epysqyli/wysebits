...
const Feed = ({
  ...
  const [favCatsNextPage, setFavCatsNextPage] = useState(favCatsPagy ? favCatsPagy.next : null);

  const [followingEntries, setFollowingEntries] = useState(followingEntriesProps);
  const [followingNextPage, setFollowingNextPage] = useState(followingPagy ? followingPagy.next : null);
  ...
  const updateFeedState = async (currentEntries, setEntriesState, pageNext, setPageState) => {
    const newEntries = await updateFeed(userState.user, currentSelection, pageNext);
  ...
  const getMoreEntries = async () => {
    ...
    if (currentSelection === "user_feed")
      await updateFeedState(globalEntries, setGlobalEntries, nextPage, setNextPage);

    if (currentSelection === "categories_feed")
      await updateFeedState(favCatsEntries, setFavCatsEntries, favCatsNextPage, setFavCatsNextPage);

    if (currentSelection === "following_feed")
      await updateFeedState(followingEntries, setFollowingEntries, followingNextPage, setFollowingNextPage);
  };
  ...
  if (userState.isLogged === true) {
    ...
    return (
      <div className='pt-10 lg:pt-16'>
        ...
        <div className='bg-feed bg-cover bg-center shadow lg:w-4/5 xl:w-2/3 2xl:w-2/5 lg:my-5 lg:rounded-md mx-auto lg:mb-20'>
        ...
        <InfiniteScroll
          ...
          hasMore={
            currentSelection == "user_feed" && nextPage !== null
              ? true
              : false || (currentSelection == "categories_feed" && favCatsNextPage !== null)
              ? true
              : false || (currentSelection == "following_feed" && followingNextPage !== null)
              ? true
              : false
          }
          endMessage={feedEnd}
