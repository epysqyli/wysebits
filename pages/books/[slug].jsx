      ...
      <div className='pt-10 lg:pt-16'>
        ...
        {pagy.prev === null && pagy.next === null ? null : (
          <div className='flex justify-around my-16 md:w-4/5 lg:w-1/2 mx-auto'>
            <div className='w-1/3'>
              <PageNavButton direction='left' clientUrl={clientUrl} url={pagy.prev_url} />
            </div>
            <div className='w-1/3'>
              <PageNavButton direction='right' clientUrl={clientUrl} url={pagy.next_url} />
            </div>
          </div>
        )}
        ...
        {recommendations.length !== 0 ? (
          <div className='mt-5 lg:mt-16'>
            <div className='w-4/5 md:w-5/6 xl:w-4/6 2xl:w-1/2 mx-auto flex items-center justify-around border-t pt-5 lg:pt-10'>
              <Bookmark className='w-1/5' size={48} strokeWidth={1.5} color='white' />
              <div className='w-3/5 text-2xl lg:text-3xl text-gray-50'>
                Recommeded books from the {book.category.name} category
              </div>
            </div>
            <div className='mx-auto pt-10 pb-16 w-11/12 md:w-5/6 xl:w-4/6 2xl:w-1/2 grid grid-cols-1 md:grid-cols-2 gap-x-2 lg:gap-x-10 gap-y-10 mt-10'>
        ...
        {userState.isLogged ? (
          <Link href={`/users/book-tiles/create/${book.id}`}>
            <div className='border px-5 pt-3 md:px-8 md:py-5 mt-20 lg:mt-32 bg-gray-50 rounded-md shadow group transition-all cursor-pointer hover:shadow-md text-center lg:w-4/5 mx-auto'>
              Be the first and share your knowledge by adding your own insights for <b>{title}</b> now!
              <FilePlus
          ...
          <Link href='/registrations/signup'>
            <div className='border px-5 pt-3 md:px-8 md:py-5 mt-20 lg:mt-32 bg-gray-50 rounded-md shadow group transition-all cursor-pointer hover:shadow-md text-center lg:w-4/5 mx-auto'>
              Signup to be the first to share your knowledge by adding your own insights for <b>{title}</b>{" "}
              now!
              <UserPlus
                className='mt-10 mb-2 mx-auto group-hover:scale-110 transition-all group-active:text-gray-400'
                size={36}
