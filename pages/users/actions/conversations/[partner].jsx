...
export const getServerSideProps = async (context) => {
  ...
  const [messages, avatar] = await Promise.all([getMessages(conversationId, context), getAvatar(partnerId)]);
...
const Conversation = ({ messages, partner, userState, conversationId, avatarUrl }) => {
      ...
      <div>
        ...
        <div className='mt-10 lg:w-4/5 mx-auto h-1/2 max-h-96 overflow-auto px-3 shadow-inner rounded-md'>
          {messagesState.map((msg) => {
            return (
              <Message message={msg} isLastMsg={isLastMsg} currentUserId={userState.user.id} key={msg.id} />
            );
          })}
        </div>
      </div>
