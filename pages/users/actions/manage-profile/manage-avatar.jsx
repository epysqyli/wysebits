...
import { CheckCircle, Image as ImageLoader, XCircle, Upload, AlertCircle } from "react-feather";
...
const ManageAvatar = ({ userState, userLoading }) => {
  ...
  const isBtnActive = () => {
    if (avatarUrl !== userState.user.avatar && avatarUrl !== null && fileAllowed === true) return true;
  ...
  return (
    <div className='pt-10 lg:pt-16'>
      ...
      <div className='w-4/5 mx-auto my-10'>
        <div className='text-center mb-20 text-2xl border-b pb-2 text-gray-50'>Current profile picture</div>

        {fileAllowed === true ? (
          <div className='flex items-center justify-around relative h-60'>
            <div>
              {avatarUrl === null || userLoading === true ? (
                <ImageLoader size={60} strokeWidth={1.5} className='text-gray-50' />
              ) : (
                <Image src={avatarUrl} layout='fill' objectFit='contain' className='rounded-md' />
              )}
            </div>
          </div>
        ) : (
        ...
        <form className='mt-10 mb-20' onSubmit={handleSubmit}>
          <div className='flex items-center justify-around md:w-3/5 lg:w-2/5 mx-auto'>
            ...
            <input type='file' name='avatar' id='new-image' className='hidden' onChange={handleFileUpload} />
          </div>
