...
export const getServerSideProps = async (context) => {
  if (isLogged(context)) {
    ...
    if (context.query.searchTerms === undefined) {
      ...
      books = await searchWithinFavBooks(loggedUser.data.user.id, keywords, pageNum);
    }
...
const FavoriteBooks = ({ books, pagy, userState, currentSearchTerms, searchParams }) => {
  const clientUrl = "/users/actions/favorite-books";

  const slug = (title, id) => slugify(`${title}-${id}`, { lower: true, strict: true });
