import { UserPlus, UserMinus, ThumbsUp, ThumbsDown, Heart, MessageCircle, X } from "react-feather";
...
const EntryActions = ({
  ...
  const basicStyle = "flex justify-between items-center text-sm px-10 md:px-16 py-4";

  return (
    <div className={feed ? feedStyle : basicStyle}>
      <div className='flex justify-center items-center gap-x-3'>
        {isUpvoted(upvotedEntries, entryProp) ? (
          <div onClick={() => removeUpvoteAndUpdateState(user, entryProp, upvotedEntries, setUpvotedEntries)}>
            <ThumbsUp
              size={16}
              fill='darkgray'
              color='darkgray'
        ...
        {isDownvoted(downvotedEntries, entryProp) ? (
          <div
            onClick={() =>
              removeDownvoteAndUpdateState(user, entryProp, downvotedEntries, setDownvotedEntries)
            }
          >
            <ThumbsDown
              size={16}
      ...
      <div className='flex justify-around items-center w-1/4'>
        <div>
          {isFavInsight(entryProp, insights) ? (
            <div onClick={() => removeFromFavInsightsAndUpdateState(user, entryProp, insights, setInsights)}>
              <Heart
                size={16}
                fill='gray'
                color='darkgray'
            ...
            <div onClick={() => addToFavInsightsAndUpdateState(user, entryProp, insights, setInsights)}>
              <Heart
                size={16}
                color='darkgray'
                className='transition-all hover:scale-105 active:scale-125 cursor-pointer'
        ...
        <div className='flex items-center'>
          {commentsView ? (
            ...
            <MessageCircle
              ...
              color='darkgray'
              className='ml-1 transition-all hover:scale-105 active:scale-125 cursor-pointer'
              onClick={() => loadComments(entryProp.id, setComments, showComments)}
            />
          )}
        </div>
      </div>
        ...
        {user.id !== entryUser.id ? (
          isFollowed(followedUsers, entryUser) ? (
            <div
              ...
              onClick={() => unfollowAndUpdateState(user, entryUser, followedUsers, setFollowedUsers)}
            >
              <UserMinus size={16} />
            </div>
          ) : (
            <div
              className='cursor-pointer hover:scale-110 text-gray-600'
              onClick={() => followAndUpdateState(user, entryUser, followedUsers, setFollowedUsers)}
            >
              <UserPlus size={16} />
            </div>
          )
