import { BookOpen, AlignCenter, Users, UserPlus, UserMinus, Send, ArrowUpRight } from "react-feather";
...
const BasicInfo = ({ user, following, setFollowedUsers, userState }) => {
  ...
  const sendMessage = async () => {
    const resp = await findOrCreateConversation(userState.user.id, user.user.id);
    ...
    <>
      <div className='md:flex items-center justify-around'>
        <div className='flex justify-around items-center gap-x-5'>
          ...
          <div>
            <div className='text-5xl font-bold text-gray-100'>{user.user.username}</div>
          </div>
        </div>
        ...
        <div className='flex items-center justify-around mt-10 md:block'>
          ...
          >
            <div className='flex items-center gap-x-2 group p-1 cursor-pointer hover:shadow-md hover:rounded-md active:shadow-inner'>
              ...
              <div className='text-gray-200 md:text-gray-100'>{user.book_tiles.length} books</div>
              <ArrowUpRight
                size={18}
          ...
          <Link href={`#`}>
            <div className='flex items-center gap-x-2'>
              ...
              <div className='text-gray-200 md:text-gray-100'>{user.followers.length} followers</div>
            </div>
          </Link>
        </div>
      </div>

      {userState.user.id === user.user.id || userState.isLogged === false ? null : (
        <div className='flex justify-around items-center mt-10 mx-auto md:w-3/5 lg:w-2/5'>
          {userState.isLogged && userState.user.id !== user.user.id ? (
            isFollowed(following, user.user) ? (
              <div
                onClick={() => unfollowAndUpdateState(userState.user, user.user, following, setFollowedUsers)}
              ...
              >
                ...
                />
                <div className='text-gray-700 group-hover:text-black'>Unfollow user</div>
              </div>
            ) : (
              <div
                onClick={() => followAndUpdateState(userState.user, user.user, following, setFollowedUsers)}
                className='flex items-center justify-center gap-x-5 cursor-pointer p-2 rounded-md shadow bg-white group active:shadow-inner'
              >
                <UserPlus
                  size={20}
                  className='text-gray-600 group-hover:scale-90 group-active:scale-75 transition-transform'
                />
                <div className='text-gray-700 group-hover:text-black'>Follow user</div>
              </div>
            )
          ) : null}
          ...
          >
            <Send
              ...
              className='text-gray-600 group-hover:scale-90 group-active:scale-75 transition-transform'
            />
            <div className='text-gray-700 group-hover:text-black'>Send Message</div>
          </div>
        </div>
      )}
    </>
